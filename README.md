...
| `pg_timers.action_timeout_ms` | `0` | sighup | Statement timeout applied to each timer action (ms). `0` disables the limit. Set to e.g. `30000` to kill runaway queries after 30 seconds. |
...
**Important:** Any user with `EXECUTE` on the scheduling functions can cause arbitrary SQL to run under their own identity via the background worker — even after their session ends. Use `pg_timers.action_timeout_ms` to limit runaway queries.
